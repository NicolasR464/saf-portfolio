                        ...
                        videoNew
                            ...
                            .then((video) => {
                                ...
                                if (video.regionRestricted) {
                                    let countries = ''
                                    video.regionRestricted.forEach(
                                        (country, i) => {
                                            countries +=
                                                i ==
                                                video.regionRestricted.length -
                                                    1
                                                    ? ' and ' + getName(country)
                                                    : i === 0
                                                    ? getName(country)
                                    ...
                                    req.flash(
                                        'error',
                                        'This video is set up to be blocked in ' +
                                            countries
                                    )
                                }
    ...
    if (vidPlr == 'yt') {
        ...
        axios
            ...
            .then((info) => {
                ...
                isRestricted =
                    info?.data?.items?.[0]?.contentDetails?.regionRestriction

                if (idExists == 1) {
                    info?.data?.items?.[0]?.contentDetails?.contentRating
...
exports.postlogin = (req, res) => {
    ...
    if (
        email.trim() !== process.env.SAF_EMAIL.trim() &&
        email.trim() !== process.env.DEV_EMAIL.trim()
    ) {
        console.log('❌ email')
